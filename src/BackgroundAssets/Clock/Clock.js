...
export const Clock = () => {
  ...
  const setDate = () => {
    ...
    const now = new Date();
    if (!hourHand || !secondHand || !minuteHand) return;
  ...
  return (
    <div className="clock">
      <div className="clock-face" ref={clockFaceRef}>
        ...
        <Typography className="clock-num twelve" sx={{ fontSize: "2rem" }}>
          XII
        </Typography>
        <Typography variant="h4" className="clock-headline">
          Meet me at <br /> Midnight
        </Typography>
      </div>
    </div>
  );
